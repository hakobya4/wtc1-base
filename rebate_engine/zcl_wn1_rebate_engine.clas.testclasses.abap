CLASS ltc_compute_rebate DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA m_cut TYPE REF TO zcl_wn1_rebate_engine.
...
CLASS ltc_compute_rebate IMPLEMENTATION.
  ...
  METHOD setup.
    "Given
    DATA(rules) = VALUE zcl_wn1_rebate_engine=>tt_rules(
      ( NEW zcl_wn1_rule_item_id( i_item_id = 1  i_req_quantity = 2  i_rebate = 5 ) ) ).

    m_cut = NEW zcl_wn1_rebate_engine( rules ).
  ENDMETHOD.
