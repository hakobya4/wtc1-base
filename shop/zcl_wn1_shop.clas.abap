...
PROTECTED SECTION.
  PRIVATE SECTION.
    ...
    DATA:
      m_cart    TYPE REF TO ZCL_WN1_SHOP_CART,
      m_catalog TYPE REF TO ZCL_WN1_SHOP_CATALOG.
...
CLASS ZCL_WN1_SHOP IMPLEMENTATION.


  METHOD build_new_empty_cart.
    m_cart = NEW ZCL_WN1_SHOP_CART( ).
  ...
  METHOD constructor.
    m_catalog = NEW ZCL_WN1_SHOP_CATALOG( ).
  ...
  METHOD zif_wtc_shop~buy_cart.
    DATA(payment_processor) = NEW ZCL_WN1_SHOP_PAYMENTPROCESSOR( ).
    rv_success = payment_processor->pay(
        i_email_sender = NEW ZCL_WN1_SHOP_EMAIL_SENDER( )
