...
CLASS ltc_constructor_injection DEFINITION FOR TESTING
  RISK LEVEL HARMLESS
  DURATION SHORT.

  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_wn1_constructor_injection.
...
CLASS ltc_constructor_injection IMPLEMENTATION.
  ...
  METHOD setup.
    cut = NEW zcl_wn1_constructor_injection( i_cash_provider = NEW ltd_cash_provider_double( ) ).
  ENDMETHOD.
